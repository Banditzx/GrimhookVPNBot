...
В Docker/Portainer база SQLite хранится в volume `grimhook_vpn_data` по пути `/app/data/users.db`. Не удаляйте этот volume при обновлении контейнера, иначе потеряются пользователи, подписки, платежи и сообщения.
...
* `XUI_MANAGED_CLIENT_PREFIX` — префикс клиентов 3x-ui, которыми разрешено управлять боту. По умолчанию `user_`; ручные профили без этого префикса бот не удаляет, не продлевает и не обновляет.
...
Проект переработан с использованием базы [HOLKus/3X-UI-TGShopBot](https://github.com/HOLKus/3X-UI-TGShopBot). Произведен редизайн, добавлена торговля через Telegram Stars и новый функционал для администрирования, платежей и поддержки пользователей.
