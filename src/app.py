...
async def sync_vpn_expiries():
    """Синхронизирует сроки клиентов в 3x-ui с датами подписок из базы."""
    users = await get_all_users()
    synced = 0
    failed = 0

    for user in users:
        if not user.vless_profile_data or not user.subscription_end:
            continue
        ...
        try:
            ...
            failed += 1
            logger.error(f"❌ VPN expiry sync error for {user.telegram_id}: {e}")

    logger.info(f"✅ VPN expiry sync completed: synced={synced}, failed={failed}")
...
async def main():
    ...
    try:
        ...
        # Синхронизируем сроки 3x-ui клиентов с базой после возможных оплат/ручных правок
